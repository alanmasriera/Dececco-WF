...
Public Class frmABMActividad
    ...
    Private Sub cargarGrillaActividades(ByVal filtro_nombre As String)
        Dim dt As List(Of Entidades.Tipo_actividad)
        Dim row_act As Integer
        dt = ControladorActividades.ObtenerTiposActividad(filtro_nombre)

        Me.dgv_actividad.Rows.Clear()

        'Asigno los valores a la grilla
        For Each row As Entidades.Tipo_actividad In dt
            row_act = Me.dgv_actividad.Rows.Add()
            Me.dgv_actividad.Rows(row_act).Cells("id_actividad").Value = row.id_tipo_actividad
            Me.dgv_actividad.Rows(row_act).Cells("porcentaje").Value = row.criticidad
            Me.dgv_actividad.Rows(row_act).Cells("descripcion").Value = row.descripcion
            Me.dgv_actividad.Rows(row_act).Cells("logueo_habilitado").Value = row.logueo_habilitado
            Me.dgv_actividad.Rows(row_act).Cells("Asignable").Value = row.asignable
            Me.dgv_actividad.Item(eficiancia_dgv.Index,row_act).Value = row.porc_eficiencia
        Next
    ...
    Private Sub btn_modificar_Click(sender As Object, e As EventArgs) Handles btn_modificar.Click
        ...
        Me.numEficiencia.Value = CDec(Me.selected_row.Cells.Item(eficiancia_dgv.Index).Value)
    ...
    Private Sub btn_guardar_Click_1(sender As Object, e As EventArgs) Handles btn_guardar.Click
        If (selected_row Is Nothing) = False Then
            If String.IsNullOrEmpty(txt_nombre_actividad.Text) = False Then
                ...
                Dim aux_tipo_act As New Entidades.Tipo_actividad

                With aux_tipo_act
                    .id_tipo_actividad = CInt(selected_row.Cells("id_actividad").Value)
                    .criticidad = txtnum_peso.Value
                    .descripcion = txt_nombre_actividad.Text
                    .logueo_habilitado = chk_permitir_logueo.Checked
                    .asignable = chk_asignable.Checked
                    .porc_eficiencia = numEficiencia.Value
                End With


                ControladorActividades.ActualizarTipoActividad(aux_tipo_act)

                oTipo_actividad.ActualizarAsignableActividadProyecto(aux_tipo_act.id_tipo_actividad, aux_tipo_act.asignable)
