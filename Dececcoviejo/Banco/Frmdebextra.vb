Public Class Frmdebextra

    Private Sub Frmdebextra_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub ObtenerDatos()
        Me.txtBoleta.Text = oDebito.comprobante
        Me.txtDetalle.Text = oDebito.detalle
        Me.dpFecha.Value = oDebito.fecha
        Me.Txtimporte.Text = CStr(oDebito.importe)
        Me.txtiva.Text = CStr(oDebito.iva)
    End Sub

    'ASIGNA LOS VALORES DEL FORMULARIO AL EMPLEADO
    Private Sub AsignarDatos()
        oDebito.Comprobante = Me.txtBoleta.Text
        oDebito.Detalle = Me.txtDetalle.Text.Trim
        oDebito.Fecha = Me.dpFecha.Value.Date
        oDebito.importe = CDec(Me.Txtimporte.Text)
        oDebito.id_banco = CInt(CType(Me.Owner, Frmmovbancos).lblidBanco.Text)
        oDebito.id_cuenta = CInt(CType(Me.Owner, Frmmovbancos).lblidCuenta.Text)
        oDebito.id_empresa = CInt(CType(Me.Owner, Frmmovbancos).lblidEmpresa.Text)
        oDebito.periodo = Me.dpFecha.Value.Date
        oDebito.iva = CDec(Me.txtiva.Text)
    End Sub
    ...
    Private Sub Guardar()
        Try
            ...
            Select Case BanderaCheque
                Case 5 'GUARDA,REFRESCA LA GRILLA E INSERTA UNO NUEVO (AGREGAR)
                    oDebito.Guardar()
                    ...
                    Me.LimpiarControles()
                    Me.dpFecha.Focus()
                Case 55 'GUARDA Y SALE (MODIFICAR)
                    oDebito.Guardar()
                    Me.Close()
            End Select
        Catch ex As Exception
    ...
    Private Sub btnGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGuardar.Click
        ...
        'controla la fecha_debito con el período indicado en Movimiento Bancos.
